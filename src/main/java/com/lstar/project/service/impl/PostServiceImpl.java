package com.lstar.project.service.impl;

import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.lstar.project.common.Code;
import com.lstar.project.exception.AppException;
import com.lstar.project.mapper.PostMapper;
import com.lstar.project.model.entity.Post;
import com.lstar.project.model.enums.PostGenderEnum;
import com.lstar.project.model.enums.PostReviewStatusEnum;
...
public class PostServiceImpl extends ServiceImpl<PostMapper, Post> implements PostService {
    ...
    public void validPost(Post post, boolean add) {
        ...
        if (StringUtils.isNotBlank(content) && content.length() > 8192) {
            throw new AppException(Code.PARAMS_ERROR, "内容过长");
        }
        if (reviewStatus != null && !PostReviewStatusEnum.getValues().contains(reviewStatus)) {
            throw new AppException(Code.PARAMS_ERROR);
        }
        if (age != null && (age < 18 || age > 100)) {
            throw new AppException(Code.PARAMS_ERROR, "年龄不符合要求");
        }
        if (gender != null && !PostGenderEnum.getValues().contains(gender)) {
            throw new AppException(Code.PARAMS_ERROR, "性别不符合要求");
        }
    }
}
